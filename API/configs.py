ENDPOINT_KNOWLEDGE_GRAPH_URL = "http://Ozans-MacBook-Pro-7.local:7200/repositories/vm3a3" # Knowledge graph endpoint can be a triples store http endpoint URL or a filepath containing the triples.
...
MAX_SCORE_PARSER_TRIPLES_FAISS = 0.15 # Maximum acceptable score for hits using the FAISS index
MIN_SCORE_PARSER_TRIPLES_WOOSH = 0.15 # Minimun acceptable score for hits using the WOOSH index
...
LLM_MODEL =  "lbl/llama" #"" #"llama3-70b-8192" # Exemples: "gpt-3.5-turbo","gpt-3.5-turbo-16k","gpt-4","gpt-4o","gpt-4-turbo","gpt-4o-mini"
TEMPERATURE_TRANSLATE = 0  # LLM temperature to generate SPARQL
SIZE_CONTEXT_WINDOW_TRANSLATE = 7 # Number of messages passed by LLM when generating SPARQL
TEMPERATURE_SELECT = 0 # LLM temperature to select best SPARQL
SIZE_CONTEXT_WINDOW_SELECT  = 7 # Number of messages passed by LLM when selecting the best SPARQL
TEMPERATURE_FINAL_ANSWER = 0 # LLM temperature to generate final answer in natural language
...
RELEVANCE_THRESHOLD = 0.25 # Threshhold for filter triples based on question embedding distance vector
MAX_HITS_RATE = 0.0005 # percentage of the total triples allowed to be passed in the subgraph triples filter
PRINT_HITS = True # Show list of triples marked as relevant to the query according to vector distance

VISUALIZATION_TOOL_URL="http://localhost:7200/repositories/databricks" # URL for a KG navigation tool displayed when clicking on URI links
