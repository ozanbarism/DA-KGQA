...
class Filter_Triples:
    ...
    def filter_triples_relevance(self, question, print_hits=False,needed_nodes=None,needed_properties=None):
        ...
        with warnings.catch_warnings():
            ...
            hits = []
            #print("Similarity scores below threshold by FAISS:")
            #print("self.relevance_threshold: ", self.relevance_threshold)
                
            with open('relevant_triples_with_scores.txt', 'w', encoding='utf-8') as file:
                for hit in hits_:
                    if hit[1] >= self.relevance_threshold:
                        #print(f"Triple: {hit[0]}, Similarity Score: {hit[1]}")
                        hits.append(hit[0])
                        # Save the triple and its similarity score
                        file.write(f"Triple: {hit[0]}, Similarity Score: {hit[1]}\n")
                    else:
                        break
            if len(hits) == 0:
        ...
        if hits != None and len(hits) > 0:
            ...
            for property in properties:
                ...
                selected_triples+= list(metadata_graph)
                        ...
                        # Save selected triples to a text file
            with open('relevant_triples.txt', 'w', encoding='utf-8') as file:
                for triple in set(selected_triples):
                    file.write(f"{triple}\n")
